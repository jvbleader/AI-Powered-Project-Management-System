...
from schemas.user_schema import UserLogin, ChangePassword
...
def logout(response: Response, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    ...
    db.query(RefreshToken).filter(RefreshToken.user_id == current_user.id).update({RefreshToken.revoked: True})
    db.commit()

    return {"message": "Đăng xuất thành công"}

@router.get("/refresh")
def refresh_access_token(response: Response, db: Session = Depends(get_db), refresh_token: str | None = Cookie(None)):
...
def change_password(data: ChangePassword, response: Response, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.query(User).filter(User.id == current_user.id).update({User.password_hash: hashed_new_pass})
