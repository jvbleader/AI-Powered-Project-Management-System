...
def get_project_member_by_id(db: Session, member_id: int, project_id: int) -> Optional[ProjectMember]:
    return db.query(ProjectMember).filter(
        ProjectMember.id == member_id,
        ProjectMember.project_id == project_id
    ).first()

def get_project_member_with_role(db: Session, project_id: int, user_id: int, role_id: int) -> Optional[ProjectMember]:
    return db.query(ProjectMember).filter(
        ProjectMember.project_id == project_id,
        ProjectMember.user_id == user_id,
...
def list_project_members(db: Session, project_id: int, search: Optional[str] = None):
    query = (
